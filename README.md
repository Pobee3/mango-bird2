...
保存成功后，只要没有再次出现“请输入 API Key”、或在“更换模型”后再次提示“请输入 API Key”等红色提示，之后你在聊天框中输入的内容都会作为正常问题发送给你选择的大模型服务商，例如 DeepSeek 或 GLM，用于生成回答。

请不要在聊天中输入密码、身份证、商业机密等敏感信息。也不建议把 API Key 直接发给 agent。

## API与费用
模型调用会消耗你自己的模型服务账号额度，具体费用以 DeepSeek / GLM 等服务商规则为准。（这两个国产模型好用便宜，应对基础问答绰绰有余）
...
- 可以移动面板或者小鸟位置，如果是番茄钟简易按钮模式，选中方框仅暂停，需要选中小鸟才移动
