...
- 普通用户：让 Codex或Claude code安装 skill，并自动下载已经打包好的 macOS app。
- 开发者：阅读 skill、Swift 桌面壳、网页互动逻辑和开发说明，可以知道这个桌宠是怎么实现的。
...
## 快速安装
通过分享的zip链接直接解压后，双击或者右键单击打开时会会跳出：
如果 macOS 提示Apple无法验证“mango-bird”是否包含可能危害Mac安全或泄漏隐私的恶意软件，或无法验证开发者：
1. 先右键 app，选择“打开”
2. 如果仍然拦截，打开“系统设置 → 隐私与安全性”，找到软件，点“仍要打开”

注：弹出该消息是因为它不是 Apple 认可开发者签名并公证过的 app

Codex部分：
让 Codex 负责下载和安装，你自己负责启动。你可以直接复制这段话给 Codex：
...
这里的 `$HOME` 表示当前用户的个人目录，例如 `/Users/your-name`（这样做的好处是，同一个电脑哪怕你换一个用户，那个用户也能使用）。`open` 的作用是让 macOS 打开这个 app。
...
⚠️如果安装好后你关闭了桌宠，第二天想打开它找不到，你可以点开访达搜索mango-bird，或者在点开左侧菜单栏，那里有个三角支架形的“应用程序”，点进去后找到app，里面会展开被收缩在一块的其他app，然后再次查找这个桌宠。

## macOS 桌宠应用

本项目包含原生 macOS 轻量封装，不需要 Electron。当前桌面版只支持 macOS；Windows、Linux 或其他非 macOS 电脑可以下载仓库阅读 skill 和开发说明，但不能直接运行这个桌面 app。

当前 app 为 Swift 原生本地代理。直接下载 app 的用户不需要额外安装 Python runtime，也不需要 Xcode 或 Command Line Tools 才能运行。

当前 release app 未签名、未公证（因为是个人开发且不准备商用之类的，大家就图一乐，所以我不打算像正式发布app那样做签名公证啥的）。面向正式发布时，建议使用 Apple Developer ID 签名并 notarize，这样普通用户就不需要处理 Gatekeeper 提示。
...
注意，这里虽然写的deepseek,但它同时支持 `deepseek` 和 `glm`（这俩国产的好用且便宜）。如果不配置 Key，桌宠仍可运行，只有“问小鸟”聊天会提示缺少 Key。

默认模型使用 `deepseek-v4-flash`，GLM 使用 `glm-4-flash`。因为在这里聊天只是为了临时起意想知道一些碎片知识，不形成专题和长期记忆，所以用响应最快速的模型模式。
...
- 右键点击鸟身会在小鸟右侧弹出白色小框，可选择“回巢”退出桌宠。
- 光标停在小鸟头上会触发蹭蹭。
- 单击小鸟会弹出高透明磨砂工具框；双击小鸟查看当前位置天气，右键天气泡泡会重新定位并刷新本地天气（需要等个2-3s）。工具框打开后，小鸟会缩小并跳到框的右上沿。
