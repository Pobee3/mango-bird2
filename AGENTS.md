...
Mango Bird 是一个浏览器内运行的芒果小鸟桌宠互动原型。用户点击芒果后，小鸟会从芒果中孵化出来，并通过逐帧 PNG、CSS 动画和 JavaScript 状态控制完成摸头蹭蹭、追逐啄击、天气气泡、临时 AI 问答、健康计时提醒、待机眨眼、睡觉和唤醒等互动。

当前版本无需构建工具，核心 HTML、CSS 和 JavaScript 都位于 `mango-bird.html`。AI 问答通过 `mango-bird-server.py` 提供本地静态服务和 DeepSeek 代理。
...
天气功能需要联网，并会优先调用浏览器定位权限显示当前位置天气。若定位不可用或被拒绝，才使用上海作为后备坐标。

AI 聊天需设置个人 DeepSeek Key 并改用本地服务启动：
...
默认使用 `deepseek-v4-flash` 并关闭 thinking 使用快速模式；快速模式请求不会向 DeepSeek 发送 `thinking` 字段。聊天页不显示模型与模式切换控件。该配置只影响 Mango Bird 本地服务，不改变用户在其他软件中使用同一 API Key 时选择 Pro 或 thinking 的能力。
...
6. 右键点击舞台、芒果或小鸟会在小鸟右上角显示淡蓝灰色圆角弹出框，位置与左上角提醒框近似对称；选择“回巢”会调用 `closeMangoPet()` 隐藏整个桌宠。当前网页原型可用“再来一次”恢复，未来 Mac 桌面版可把该入口接到真正退出应用。
...
- 单击小鸟会在小鸟附近打开高透明磨砂工具框，默认显示“问小鸟”，也可切换到“计时提醒”。为区分双击天气，面板会在约 260 毫秒后打开。小鸟缩小后沿弧线跳到框内的右上区域，身体向面板内侧收进，双脚明确落在面板内部，面朝左侧静止站立；停驻时不显示地面阴影，也不沿用待机悬浮效果。
...
- 使用 DeepSeek 完成临时知识问答，默认使用中文、简洁且可靠地回答。
- Mango 的普通知识回答默认不使用表情，仅在鼓励、安慰、庆祝或轻松闲聊等合适场景偶尔使用一个，避免每轮固定或连续重复。
- 聊天页不提供模型与模式切换控件；前端固定发送 `deepseek-v4-flash` 与 `fast`，后端默认值也与此一致。服务端仅在 `thinking` 模式下发送 DeepSeek 的 `thinking` 字段，快速模式不发送该字段。
- 系统提示要求 Mango 使用自然纯文本回答，不使用 Markdown 加粗、标题、表格或代码块，尤其避免输出 `**`，除非用户明确要求格式化内容。
- Enter 发送，Shift+Enter 换行；请求期间显示“小鸟正在想…”。
- 只向模型携带当前页面最近 12 条消息，约等于 6 轮对话。
- 不使用 `localStorage`，刷新页面或点击“再来一次”会清空对话。
- “清空对话”会中止当前请求并立即清除当前会话。
- 缺少或无效的 `DEEPSEEK_API_KEY`、网络超时和额度限制会在输入框下方显示错误。
...
- 健康计时：`openTimerDialog()`、`startReminderTimer()`、`addCustomReminder()`、`scheduleNextReminder()`、`finishReminderTimer()`。
